...
/// - `color_mode` — `"color"` traces the full palette (logos, illustrations),
///   `"bw"` flattens to binary (line art, icons).
/// - `filter_speckle` — drops clusters smaller than `n×n` px. Higher = cleaner,
///   lower = preserves fine detail. VTracer default is `4`.
/// - `corner_threshold` — degrees below which a vertex is simplified to a
///   straight segment. VTracer default is `60`.
